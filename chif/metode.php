    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <tr>
                ...
                CASE 
                    WHEN nk.sifat = 'Cost' THEN MIN(n.normalisasi)
                    WHEN nk.sifat = 'Benefit' THEN MAX(n.normalisasi)
                END AS nilai
            ...
            <tr>
                ...
                $query = mysqli_query($conn, "SELECT nk.id_kriteria, 
                CASE 
                    WHEN nk.sifat = 'Cost' THEN MAX(n.normalisasi)
                    WHEN nk.sifat = 'Benefit' THEN MIN(n.normalisasi)
                END AS nilai
                FROM tbl_kriteria nk
                LEFT JOIN tbl_nilai n ON nk.id_kriteria = n.id_kriteria
                WHERE nk.id_kriteria IN (SELECT id_kriteria FROM tbl_nilai WHERE id_alternatif = '" . $id_alternatif . "')
                        ...
                        while ($a = mysqli_fetch_array($data)) {
                            ...
                            <tr>
                                ...
                                while ($result = mysqli_fetch_array($query)) {
                                    ...
                                    // Ambil informasi sifat kriteria
                                    $query_kriteria = mysqli_query($conn, "SELECT sifat FROM tbl_kriteria WHERE id_kriteria='" . $result['id_kriteria'] . "'");
                                    $result_kriteria = mysqli_fetch_assoc($query_kriteria);

                                    // Tentukan jenis fungsi agregasi berdasarkan sifat kriteria
                                    if ($result_kriteria['sifat'] == 'Cost') {
                                        $aggregation_function = 'MIN';
                                    } else {
                                        $aggregation_function = 'MAX';
                                    }

                                    // Query untuk mengambil nilai maksimum atau minimum normalisasi
                                    $query6 = mysqli_query($conn, "SELECT $aggregation_function(normalisasi) as max_nm FROM tbl_nilai
                                WHERE id_kriteria='" . $result['id_kriteria'] . "' ORDER BY id_kriteria");

                                    $result4 = mysqli_fetch_assoc($query6);
                        ...
                        while ($a = mysqli_fetch_array($data)) {
                            ...
                            <tr>
                                ...
                                while ($result = mysqli_fetch_array($query)) {
                                    ...
                                    WHERE id_kriteria='" . $result['id_kriteria'] . "' ORDER BY id_kriteria");
                                    $query_kriteria2 = mysqli_query($conn, "SELECT sifat FROM tbl_kriteria WHERE id_kriteria='" . $result['id_kriteria'] . "'");

                                    $result_kriteria2 = mysqli_fetch_assoc($query_kriteria2);

                                    // Tentukan jenis fungsi agregasi berdasarkan sifat kriteria
                                    $aggregation_function2 = ($result_kriteria2['sifat'] == 'Cost') ? 'MAX' : 'MIN';

                                    // Query untuk mengambil nilai maksimum atau minimum normalisasi
                                    $query6 = mysqli_query($conn, "SELECT $aggregation_function2(normalisasi) as min_nm FROM tbl_nilai
                                                                    WHERE id_kriteria='" . $result['id_kriteria'] . "' ORDER BY id_kriteria");

                                    $result4 = mysqli_fetch_array($query6);
